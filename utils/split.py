...
"""
def split_and_save_data(image_arrays: dict, image_captions: dict, save_folderpath=None, train_size: float=0.8, val_size: float=0.1, test_size: float=0.1):
    image_filenames = list(image_arrays.keys()) # splitting the dataset into train, validation, and test sets
    train_filenames, temp_filenames = train_test_split(image_filenames, test_size=1-train_size, random_state=42)
    val_filenames, test_filenames = train_test_split(temp_filenames, test_size=test_size/(test_size+val_size), random_state=42)
    ...
    train_captions = {filename: image_captions[filename] for filename in train_filenames}
