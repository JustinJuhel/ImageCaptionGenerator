...
def visualize_image(image_dict: dict, index: int) -> None:
    filenames = list(
        image_dict.keys()
    )  # getting the list of filenames (keys) from the dictionary

    if index < 0 or index >= len(
        filenames
    ):  # chceking if the index is within the valid range
        raise IndexError("Index out of range")

    filename = filenames[index]  # getting the filename at the specified index
    image_array = image_dict[
        filename
    ]  # getting the image array corresponding to the filename
...
def get_captions(image_captions: dict, index: int) -> list:
    ...
    captions = image_captions[
        filename
    ]  # getting the list of captions corresponding to the filename
    return captions
...
def explore_dataset(image_folder: str, captions_file: str) -> None:
    ...
    if captions_file.endswith(
        ".txt"
    ):  # determining the file type and read the captions file accordingly
    ...
    if len(image_files) < 9:  # checking if there are at least 9 images in the folder
        ...
        for i, image_file in enumerate(
            ...
            with Image.open(image_path) as img:
                ...
                if (
                    image_file in captions
                ):  # adding captions to the subplot with text wrapping
                    wrapped_captions = [
                        textwrap.fill(caption, width=40)
                        for caption in captions[image_file]
                    ]
...
def get_descriptive_statistics(image_folder: str):
    ...
    print(
        f"Max height: {max_height}, Min height: {min_height}, Mean height: {mean_height}"
    )
