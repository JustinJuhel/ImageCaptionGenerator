...
def download_dataset(destination_folder: str="data"):
    ...
    if os.path.exists(destination_folder): # checking if the destination folder exists
        ...
        if user_input.lower() == "yes":
            ...
            shutil.copytree(source_folder, destination_folder) # copying the source folder to the destination folder
...
def load_data(images_folderpath: str, captions_filepath: str):
    image_arrays, image_captions = {}, {} # initializing dictionaries to store image arrays and captions
    captions_df = pd.read_csv(captions_filepath) # reading the captions file

    for index, row in captions_df.iterrows(): # iterating over the rows in the captions DataFrame
